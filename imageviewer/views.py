...
class GetWorkspaces(APIView):
    ...
    def get(self, request, username, format=None):
        ...
        if (Users.objects.all().filter(name=username).exists()):
            names = list(Users.objects.values_list('name', flat=True)
                         .order_by('name'))
            ...
            if (Users.objects.all().filter(name=targetname).exists()):
                ...
                if (username == targetname):
                    image_list = Image.objects.all() \
                                    .filter(user__name=targetname) \
                                    .values(
                                        'image_name',
                                        'image_oid__bucket_name',
                                        'preview_url',
                                        'user__name',
                                        'is_private',
                                        'pub_date',
                    ...
                    for i in range(len(image_list)):
                        image_list[i]['pub_date'] = str(
                            image_list[i].get('pub_date'))
                    msg = "Success"
